...
# # Define comparisons
# comparisons <- list(
#   "epi_endo_vs_endo_fib" = c("epi/endo", "endo/fib"),
#   "endo_fib_vs_fib_epi" = c("endo/fib", "fib/epi"),
#   "epi_endo_vs_fib_epi" = c("epi/endo", "fib/epi")
# )
# 
# # Hallmark and Reactome pathways
# hallmark <- msigdbr(species = "Homo sapiens", category = "H") %>% split(.$gene_symbol, .$gs_name)
# reactome <- msigdbr(species = "Homo sapiens", category = "C2", subcategory = "CP:REACTOME") %>% split(.$gene_symbol, .$gs_name)
# 
# results_list <- list()
# 
# for (comp_name in names(comparisons)) {
#   
#   types <- comparisons[[comp_name]]
#   cells_use <- colnames(all_doublets)[all_doublets$doublet_type %in% types]
#   obj <- subset(all_doublets, cells = cells_use)
#   
#   # Select correct counts layer
#   layer_use <- switch(types[1],
#                       "epi/endo" = "counts.epi",
#                       "endo/fib" = "counts.endo",
#                       "fib/epi" = "counts.fib")
#   cts <- obj[["RNA"]]@layers[[layer_use]]
#   
#   # Pseudobulk by sample
#   meta <- obj@meta.data[, c("sample_id", "doublet_type"), drop = FALSE]
#   meta$cell <- rownames(meta)
#   pb <- rowsum(as.matrix(cts), group = meta$sample_id)
#   
#   groups <- unique(meta[, c("sample_id", "doublet_type")])
#   groups <- groups[order(groups$sample_id), ]
#   group <- factor(groups$doublet_type)
#   
#   # edgeR DE
#   y <- DGEList(pb)
#   y <- calcNormFactors(y)
...
#   contrast <- makeContrasts(levels(design)[1] - levels(design)[2], levels = design)
...
#   
#   # GSEA
#   fg_hallmark <- fgsea(pathways = hallmark, stats = ranks, minSize = 10, maxSize = 500, nperm = 10000)
#   fg_reactome <- fgsea(pathways = reactome, stats = ranks, minSize = 10, maxSize = 500, nperm = 10000)
#   
#   results_list[[comp_name]] <- list(
#     DE = de_tab,
#     GSEA_Hallmark = fg_hallmark %>% arrange(padj),
#     GSEA_Reactome = fg_reactome %>% arrange(padj)
#   )
#   
#   # Plot top 10 Hallmark NES
#   top10_hall <- fg_hallmark %>% arrange(padj) %>% slice_head(n = 10)
#   ggplot(top10_hall, aes(x = reorder(pathway, NES), y = NES)) +
#     geom_col(fill = "steelblue") +
#     coord_flip() +
#     labs(title = paste0("GSEA Hallmark: ", comp_name), x = "", y = "NES") +
#     theme_minimal()
# }
# 
# 
# # --- Access results ---
# # e.g., DE table for epi/endo vs endo/fib:
# # results_list$epi_endo_vs_endo_fib$DE
# # GSEA Hallmark table:
# # results_list$epi_endo_vs_endo_fib$GSEA_Hallmark
# # Check available layers
# obj[["RNA"]]@layers

# # Merge epi + endo first, then add fib
# combined_doublets <- merge(
#   x = epi_mixed_doublets_obj,
#   y = c(endo_mixed_doublets_obj, fib_mixed_doublets_obj),
#   project = "mixed_doublets"
# )
# Idents(combined_doublets) <- "doublet_type"
# table(Idents(combined_doublets))  
# 
# 
# #comparing epi/endo doublets vs others
# Idents(combined_doublets) <- "doublet_type"  
